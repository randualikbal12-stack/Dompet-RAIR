...
function mamaV(){LK={};WK={};const tabs=`<div class="seg" id="mt">${[['mama','Uang mama'],['kos','Kos per kamar'],['disan','Disan'],['riba','Riba']].map(([k,l])=>`<button data-t="${k}" class="${k===MT?'on':''}">${l}</button>`).join('')}</div>
 ...
 if(MT==='mama'){const M=mama(today),prev=mama(MY-1+'-12-31');
  ...
  <div class="kv"><span>Selisih catatan uang mama sejak Feb 2024</span><span class="ac" data-x="${FN(mamaSisaSheet)}">${rp(M.sisa)}</span></div>
 ...
 if(MT==='kos'){const kIn=(a,b)=>income(a,b,'Uang Kos Mama'),kOut=(a,b)=>expense(a,b,'Pengeluaran Uang Kos'),names=S.acc.filter(a=>a.g==='Titipan Mama').map(a=>a.n),kb=d=>names.reduce((p,n)=>p+bal(n,d),0);let tI=0,tO=0,tV=0;
  const mrow=bl.map(x=>{const i=kIn(x.a,x.b),o=kOut(x.a,x.b),v=tabNet(x.a,x.b,true),sisa=i-o,e=x.b<today?x.b:today,kn=kb(e);tI+=i;tO+=o;tV+=v;
   const k=WKs({ids:idsOf(t=>inR(t,x.a,x.b)&&(t.sb==='Uang Kos Mama'||t.sb==='Pengeluaran Uang Kos'||isKosRd(t)))},'Uang kos · '+BULAN[x.i]+' '+MY,{lines:[['Uang kos masuk',i,{ids:idsOf(t=>inR(t,x.a,x.b)&&t.tj==='Pendapatan'&&t.sb==='Uang Kos Mama')}],['− Uang kos keluar',o,{ids:idsOf(t=>inR(t,x.a,x.b)&&t.tj==='Pengeluaran'&&t.sb==='Pengeluaran Uang Kos')}],['= Sisa bulan ini',sisa,null,1],['Disetor ke reksadana uang kos mama',sum(t=>inR(t,x.a,x.b)&&isKosRd(t)&&t.t==='K'),{ids:tabIds(x.a,x.b,true,'K')}],['− Dicairkan dari reksadana',sum(t=>inR(t,x.a,x.b)&&isKosRd(t)&&t.t==='M'),{ids:tabIds(x.a,x.b,true,'M')}],['= Masuk investasi (bersih)',v,null,1],['Uang kos yang BELUM diinvestasikan (saldo kantong + cash akhir bulan)',kn,{ids:accIds(names,e)},1]],note:'Uang kos yang belum diinvestasikan dilihat dari saldo Kantong Uang Kos Mama + Uang Kos Mama Cash. Kalau hampir Rp0, berarti semua uang kos sudah masuk investasi atau sudah terpakai untuk kebutuhan kos. "Masuk investasi" bisa lebih kecil atau minus karena reksadana dicairkan untuk bayar listrik, pegadaian, dll.'});
   return`<tr class="cl" data-x="${k}"><td>${BLN[x.i]}</td><td class="n up">${rp(i)}</td><td class="n dn">${rp(o)}</td><td class="n">${rp(sisa)}</td><td class="n">${rp(v)}</td><td class="n"><b>${rp(kn)}</b></td></tr>`}).join('');
...
function kamarSheet(k){const Y0=MY+'-01-01',Y1=MY+'-12-31',L12=[];let tot=0;
 for(let i=0;i<12;i++){const a=ds(new Date(MY,i,1)),b=mEnd(MY,i),v=sum(t=>t.kos===k&&inR(t,a,b));tot+=v;L12.push([BULAN[i]+' '+MY,v,{kos:k,from:a,to:b}])}
 why({kos:k,from:Y0,to:Y1},k+' · '+MY,{lines:L12.concat([['= Total '+MY,tot,null,1]]),note:'Pemasukan kamar ini per bulan (Rp0 = tidak ada pembayaran tercatat). Pengeluaran kos (listrik, perbaikan, pegadaian) tidak dicatat per kamar, jadi dilihat di bagian "Uang kos dipakai untuk".'})}
/* ================= INVESTASI ================= */
let IT='all';
