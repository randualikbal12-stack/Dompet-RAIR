...
function load(){try{S=JSON.parse(localStorage.getItem(KEY))}catch(e){S=null}
 ...
 S.price=S.price||{...INV.last};S.rdcur=S.rdcur||{};S.goals=S.goals||[];migrate()}
...
function home(){LK={};const now=new Date(),y=now.getFullYear(),m=now.getMonth(),a=ds(new Date(y,m,1)),b=mEnd(y,m),M=mama(today);
 ...
 <div class="card"><h3>Bulan ini — ${BULAN[m]} ${y}</h3><div class="g3">
  <div class="kp" id="hI"><div class="a">Pendapatan</div><div class="b up">${rp(tI)}</div></div><div class="kp" id="hE"><div class="a">Pengeluaran</div><div class="b dn">${rp(tE)}</div></div><div class="kp" id="hS"><div class="a">Selisih</div><div class="b">${rp(tI-tE)}</div></div></div><div class="tiny" style="margin-top:6px">Ketuk angka untuk melihat transaksinya.</div></div>
 ...
 const ttl=BULAN[m]+' '+y,idsOf=f=>S.tx.filter(t=>inR(t,a,b)&&f(t)).map(t=>t.id);
 $('hI').onclick=()=>lacak({tj:'Pendapatan',from:a,to:b},'Pendapatan · '+ttl);
 $('hE').onclick=()=>lacak({from:a,to:b,ids:idsOf(t=>t.tj==='Pengeluaran'||t.tj==='Tabungan')},'Pengeluaran + tabungan bersih · '+ttl);
 $('hS').onclick=()=>lacak({from:a,to:b,ids:idsOf(t=>t.tj!=='Pindah Uang')},'Pendapatan & pengeluaran · '+ttl);
...
function rep(){LK={};const P=period(),tree={};
 ...
 $('main').innerHTML=`<div class="card"><div class="seg" id="rpm">${['hari','bulan','tahun'].map(x=>`<button data-m="${x}" class="${x===RP.m?'on':''}">Per ${x}</button>`).join('')}</div>
  ...
  <div class="g2" style="margin-top:8px"><div class="kp" id="rI"><div class="a">Pendapatan</div><div class="b up">${rp(inc)}</div></div><div class="kp" id="rE"><div class="a">Pengeluaran</div><div class="b dn">${rp(exp)}</div></div></div>
 ...
 document.querySelectorAll('#rpm button').forEach(b=>b.onclick=()=>{RP.m=b.dataset.m;rep()});$('rI').onclick=()=>lacak({tj:'Pendapatan',from:P.a,to:P.b},'Pendapatan · '+P.lab);$('rE').onclick=()=>lacak({from:P.a,to:P.b,ids:S.tx.filter(t=>inR(t,P.a,P.b)&&(t.tj==='Pengeluaran'||t.tj==='Tabungan')).map(t=>t.id)},'Pengeluaran + tabungan bersih · '+P.lab);$('rpd').onchange=e=>{RP.d=e.target.value||today;rep()};
...
function mamaV(){LK={};const tabs=`<div class="seg" id="mt">${[['mama','Uang mama'],['kos','Kos per kamar'],['disan','Disan'],['riba','Riba']].map(([k,l])=>`<button data-t="${k}" class="${k===MT?'on':''}">${l}</button>`).join('')}</div>
 ...
 if(MT==='disan'){const D=disan(today),prevD=disan(MY-1+'-12-31');let ti=0,to=0;
  ...
  <div class="tiny" style="margin-top:6px">Masuk = uang untuk Disan (dari kk Lisa). Keluar = uang yang sudah diberikan/dipakai untuk Disan. Sisa = masuk − keluar, yaitu uang Disan yang masih kamu pegang.</div></div>
  <div class="card"><h3>Per bulan ${MY} (ketuk untuk melihat transaksinya)</h3><div class="tw"><table><tr><th>Bln</th><th class="n">Masuk</th><th class="n">Keluar</th><th class="n">Sisa</th><th class="n">Status</th></tr>
